...
var originalbalance = 100; // Set your assumed initial balance here
var runningbalance = originalbalance;
...
function main() {
  ...
  game.onBet = function () {
    if (numberOfRoundsToSkip > 0) {
      ...
      log.info(
        "Skipping round to relax, and the next " +
      ...
      if (loseStreak >= maxLossesBeforeMinBet) {
        if (game.history[0] >= config.payout.value * 100) {
          ...
          game.bet(currency.minAmount).then(function (payout) {
            ...
            if (payout > 1) {
              var netwin =
                currency.minAmount * config.payout.value - currency.minAmount;
      ...
      game.bet(currentBet, config.payout.value).then(function (payout) {
        ...
        if (currentBet < currency.minAmount) {
          log.info(
            "Current bet amount is below the minimum allowed. Betting minimum amount."
          );
        ...
        if (payout > 1) {
          ...
          if (config.loggingLevel.value != "compact") {
            LogMessage(
              "We won a net profit of: " + netwin.toFixed(8),
              "success"
            );
          }
          netProfit += netwin;
        ...
        if (config.payout.value < minimumPayout) {
          log.info(
            "Payout is below the minimum allowed. Using minimum payout."
          );
        ...
        if (config.payout.value > maximumPayout) {
          log.info(
            "Payout is above the maximum allowed. Using maximum payout."
          );
...
function GetNewBaseBet() {
  ...
  if (returnValue > currency.minAmount) {
    LogMessage(
      "Recalculating base bet to " +
        returnValue.toFixed(8) +
        " which is " +
        config.betPercentage.value +
        " percent of " +
    ...
    LogMessage(
      "The recalculated bet amount " +
        returnValue.toFixed(8) +
        " is lower than the minimum allowed bet.  Setting bet to the minimum allowable amount of " +
        currency.minAmount,
      "info"
    );
